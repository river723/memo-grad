...
import { AIProviderId, AIResponse } from '../../types';
import { API_CONFIG, AI_PROVIDERS } from '../../constants';
import StorageService from '../StorageService';

/**
 * 本地 AI 引擎（单机形态 OFFLINE_MODE 专用）。
 *
 * 自 archive/standalone-final（原单机分支终点）的 AIService.ts 取回：
 * axios 直连 provider（DeepSeek），API Key 来自用户在设置页自配的本地密钥。
 *
 * 与旧版的差异：
 * - 类名 LocalAIEngine，具名导出（不再默认导出实例）。
 * - 构造时不传配置 → 各方法执行前经 ensureConfig() 从 StorageService
 *   懒加载设置并做 30s 缓存（保存设置后调 invalidateConfigCache() 立即生效）；
 *   显式传入配置（如设置页"测试连接"）则锁定，不再读设置。
 */
...
export class LocalAIEngine {
  ...
  /** 构造时显式注入配置后置 true：后续调用不再从设置懒加载 */
  private locked: boolean;

  /** 设置缓存（30s TTL）：避免每次 AI 调用都读一遍 AsyncStorage */
  private static cfgCache: { apiKey: string; aiModel: string; at: number } | null = null;

  /** SettingsScreen 保存 AI 设置后调用，使懒加载配置立即生效 */
  static invalidateConfigCache() {
    LocalAIEngine.cfgCache = null;
  }
  ...
  constructor(config: AIServiceConfig | string = '') {
    if (typeof config === 'string') {
      ...
      this.locked = Boolean(config);
      return;
    }

    const provider: AIProviderId = config.provider && config.provider in AI_PROVIDERS ? config.provider : 'deepseek';
    const providerConfig = AI_PROVIDERS[provider];
    this.provider = provider;
    this.apiKey = config.apiKey || '';
    this.baseUrl = providerConfig.baseUrl;
    this.model = config.model || providerConfig.defaultModel;
    this.locked = Boolean(config.apiKey);
  }

  /** 未显式注入配置时，从本地设置懒加载 API Key / 模型（30s 缓存）。 */
  private async ensureConfig(): Promise<void> {
    if (this.locked) return;
    try {
      if (!LocalAIEngine.cfgCache || Date.now() - LocalAIEngine.cfgCache.at > 30_000) {
        const s = await StorageService.getSettings();
        LocalAIEngine.cfgCache = { apiKey: s.apiKey || '', aiModel: s.aiModel || '', at: Date.now() };
      }
      const cfg = LocalAIEngine.cfgCache;
      if (cfg) {
        this.apiKey = cfg.apiKey;
        this.model = cfg.aiModel || AI_PROVIDERS[this.provider].defaultModel;
      }
    } catch {
      // 读不到设置时保留当前值；后续请求会因缺 Key 失败并给出既有错误文案
    }
  }
...
1. 考研英语最常考的3-5个释义（按重要性排序，特别标注熟词僻义）
2. 每个释义配一个考研真题风格的例句
3. 1-2个形近词或易混词提醒
4. 简单的词根词缀分析（帮助记忆）
5. 难度等级建议（1-5）
